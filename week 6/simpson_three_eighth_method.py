# 2.4 Simpson's Three-Eighth Rule (Numerical Integration)
# ∫f(x)dx ≈ (3h/8)[f(x_0) + 3f(x_1) + 3f(x_2) + 2f(x_3) + 3f(x_4) + 3f(x_5) + ...]
#
# ОПИСАНИЕ:
# Правило Симпсона 3/8 — аналогично правилу 1/3, но вместо параболы используется
# кубический полином (многочлен 3-й степени). На каждые три интервала (4 точки)
# строится кубическая интерполяция, интеграл по ней считается точно.
# Название "3/8" — от коэффициента 3h/8 в формуле.
#
# Требования: число интервалов кратно 3 (4, 7, 10 точек и т.д.).
# Погрешность: O(h⁴) — того же порядка, что и правило 1/3.
# Коэффициенты: в группе из 4 точек — 1, 3, 3, 1; на стыках групп — 2.
